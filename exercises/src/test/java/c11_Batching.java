...
public class c11_Batching extends BatchingBase {
    ...
    public void batch_writer() {
        ...
        Flux<Void> dataStream = dataStream().buffer(10).flatMap(batch -> writeToDisk(batch));
    ...
    public void command_gateway() {
        ...
        Flux<Void> processCommands = inputCommandStream().groupBy(Command::getAggregateId)
                .flatMap(g->g.concatMap(cmd -> sendCommand(cmd)));
    ...
    public void sum_over_time() {
        Flux<Long> metrics = metrics()
                ...
                .concatMap(w->w.reduce(0L, (a, b) -> a + b))
                .doOnNext(sum -> System.out.println("Sum: " + sum))
                .take(10);
