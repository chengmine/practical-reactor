...
public class c13_Context extends ContextBase {
    ...
    public void pagination() {
        ...
        Flux<Integer> results = Mono.deferContextual(ct -> getPage(ct.get(AtomicInteger.class).get()))
                .doOnEach(s -> {
                    if (s.getType() == SignalType.ON_NEXT) {
                        s.getContextView().get(AtomicInteger.class).incrementAndGet();
                    } else if (s.getType().equals(SignalType.ON_ERROR)) {
                        pageWithError.set(s.getContextView().get(AtomicInteger.class).get());
                        System.out.println("Error on page: " + s.getContextView().get(AtomicInteger.class).get());
