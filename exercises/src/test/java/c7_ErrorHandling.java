...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void potato_potato() {
        Mono<String> currentUser = getCurrentUser()
                ...
                .onErrorMap(t -> new SecurityException(t));
    ...
    public void under_the_rug() {
        Flux<String> messages = messageNode().onErrorResume(throwable -> Flux.empty());
    ...
    public void have_a_backup() {
        ...
        messages = messageNode().onErrorResume(e -> backupMessageNode());
    ...
    public void unit_of_work() {
        Flux<Task> taskFlux = taskQueue()
                .flatMap(t -> t.execute().then(t.commit()).onErrorResume(throwable -> t.rollback(throwable)).thenReturn(t));
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
                ...
                .onErrorContinue((e, t) -> System.out.println(t.toString()));
    ...
    public void resilience() {
        ...
        Flux<String> content = getFilesContent()
                .flatMap(mono -> mono.doOnError((e -> System.out.println("Error: + e.getMessage()"))).onErrorResume(e -> Mono.empty())); //start from here
    ...
    public void good_old_polling() {
        //todo: change code as you need
        Flux<String> alerts = nodeAlerts().repeatWhenEmpty(f->f.delayElements(Duration.ofSeconds(1))).repeat();
